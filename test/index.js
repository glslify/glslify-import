...
const importRequire = path.join(
  __dirname,
  'fixtures',
  'import-with-require.glsl'
);
const importChunk = path.join(
  __dirname,
  'fixtures',
  'import-chunk-origin.glsl'
);
...
test('glslify-import: basic', function (t) {
  ...
  t.ok(
    /basicRequire/.exec(src),
    'basic-require.glsl imported dependency is still included'
  );
  t.end();
});
...
test('glslify-import: require paths modified', function (t) {
  ...
  t.ok(
    /subRequire1/.exec(src),
    'subfolder-1/basic-require.glsl imported dependency is still included'
  );
  t.ok(
    /subRequire2/.exec(src),
    'subfolder-2/basic-require.glsl imported dependency is still included'
  );
  t.ok(!/\#pragma glslify/.exec(src), 'no pragmas remaining');
  t.end();
});

test('glslify-import: chunk', function (t) {
  var src = glslify(importChunk, {
    transform: [require.resolve('../index.js')],
  });
